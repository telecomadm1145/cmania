...
template<typename T>
bool bisearch(T rune, std::pair<T, T>* table,size_t max)
{
	size_t num = 0;
...
export int Measure(int ucs4)
{
	...
	if (ucs4 < 127)
		return 1;
	if (bisearch(ucs4, comb, countof(comb) - 1))
		return 0;
	return 1 + (bisearch(ucs4, wide, countof(wide) - 1) ? 1 : 0);
}