...
class ManiaRuleset : public Ruleset<ManiaObject> {
	...
	OsuSampleProcessor osp;
	std::deque<OsuBeatmap::StoryboardSoundSample> StoryboardSamples;
	double offset = 0;
...
public:
	...
	virtual void Load(std::filesystem::path beatmap_path) override {
		...
		// 存储需要加载的采样
		osp.LoadSkin(skin_path); // 构建谱面采样索引(sampleset==1默认)
		osp.LoadBeatmap(parent); // 构建皮肤采样索引(sampleset==0)

		orig_bmp.StoryboardSamples > ForEach([&](const auto& str) { osp.LoadSampleEx(parent / str.path); }); // This loades extra samples for storyboard
		orig_bmp.HitObjects > ForEach([&](const auto& obj) {
			if (!obj.CustomSampleFilename.empty()) {
				osp.LoadSampleEx(parent / obj.CustomSampleFilename);
			}
		}); // This loades extra samples for storyboard
		...
		Beatmap > AddRange(Select(
					  orig_bmp.HitObjects, [&](const OsuBeatmap::HitObject& obj) -> auto {
						  ...
						  // 给物件加载 Samples
						  mo.samples > AddRange(osp.GetSampleLayered(tp.SampleBank, obj.SoundType, tp.SampleSet));

						  if (!obj.CustomSampleFilename.empty()) {
							  mo.samples.push_back(osp.GetSampleCustom(parent / obj.CustomSampleFilename));
						  }
						  ...
						  if (obj.EndTime != 0) {
							  // 加载滑动音效
							  mo.ssample = osp.GetSampleLayeredOne(tp.SampleBank, HitSoundType::Slide, tp.SampleSet);

							  // 加载 Whistle 滑动音效
							  if (HasFlag(obj.SoundType, HitSoundType::Whistle))
								  mo.ssamplew = osp.GetSampleLayeredOne(tp.SampleBank, HitSoundType::SlideWhistle, tp.SampleSet);
		...
		orig_bmp.StoryboardSamples > ForEach([&](auto s) {
			s.path = (parent / s.path).string();
			StoryboardSamples.emplace_back(s);
		});
		std::sort(StoryboardSamples.begin(), StoryboardSamples.end(), [](const auto& a, const auto& b) { return a.StartTime < b.StartTime; });
		...
		auto diff = 3; // CalculateDiff(orig_bmp, Mods, keys);
	...
	virtual void Update() override {
			...
			if (obj.HasHit && obj.EndTime != 0) {
				if (wt_mode) {
					if (time > obj.StartTime && time < obj.EndTime) {
						StartSlidingSample(obj);
						KeepSlidingSample(obj);
					}
					if (time > obj.EndTime) {
						StopSlidingSample(obj);
					}
				}
				else {
		...
		while (StoryboardSamples.size() > 0) {
			auto& front = StoryboardSamples.front();
			if (time > front.StartTime) {
				auto sample = osp.GetSampleCustom(front.path);
				auto stream = sample->generateStream();
				stream->play();
				delete stream;
				StoryboardSamples.pop_front();
			}
			else
				break;
		}
	...
	void KeepSlidingSample(ManiaObject& obj) {
	...
	void StartSlidingSample(ManiaObject& obj) {
	...
	void StopSlidingSample(ManiaObject& obj) {
	...
	double CalcFlashlight(OsuMods mods, double ratio) {
		...
		if (HasFlag(mods, OsuMods::Hidden)) {
			if (ratio < 0.5) {
				return pow(ratio / 0.5, 2);
			}
		}
		if (HasFlag(mods, OsuMods::FadeOut)) {
			if (ratio > 0.5) {
				return pow((1 - ratio) / 0.5, 2);
			}
		}
		return 1;
	}
			...
			for (auto& obj : Beatmap) {
				...
				if (obj.Column == j) {
					...
					if (obj.EndTime != 0 && !obj.HasHold) {
						...
						auto base_alpha = base.Alpha;
						for (int j = endy; j < a; j++) {
							if (j < 0 || j > buffer.Height - judge_height)
								continue;
							auto ratiofl = (double)j / (buffer.Height - judge_height);
							if (ratiofl < 0 || ratiofl > 1)
							{
								__debugbreak();
							}
							auto flashlight = CalcFlashlight(Mods, ratiofl);
							base.Alpha = base_alpha * flashlight; // now we need 2 map noddles to a ratio
							buffer.DrawLineV(i + 1, i + key_width, j, { {}, base, ' ' });
						}
