...
auto ForEach(auto func) {
	return [func](auto& cont) -> auto& {ForEach(cont, func); return cont; };
}
...
auto AddRangeSet(const auto& src) {
	return [&](auto& dest) -> auto& { AddRangeSet(dest, src); return dest; };
}
template <class Iter>
class LinqContainer {
	Iter b;
...
void First(const auto& container, auto& target) {
...
template <class Func, class Type>
concept NonFluentMethod = !FluentMethod<Func, Type> && requires(Func& f, Type& t) {
	{ f(t) } -> has;
};
