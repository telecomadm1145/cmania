...
// 请确保 outbuf 拥有安全的至少 4bytes 用于写入
export void Ucs4Char2Utf8(int codePoint,char* outbuf,int& outlen)
{
    outlen = 0;
    if (codePoint <= 0x7F)
    {
...
export std::u32string Utf82Ucs4(const std::string& utf8String)
{
...
export std::string Utf162Utf8(const std::wstring& utf16String) {
...
export bool IsMultiUtf16(wchar_t chr)
{
    return chr >= 0xD800 && chr <= 0xDBFF;
}
export bool IsMultiUtf16Tail(wchar_t chr)
{
    return chr >= 0xDC00 && chr <= 0xDFFF;
}
export std::u32string Utf162Ucs4(const std::wstring& utf16String) {
    std::u32string ucs4String;
...
export std::string UCS4ToUTF8(const std::u32string& ucs4String)
{
    std::string utf8String;
